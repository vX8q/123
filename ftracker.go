...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Плавание":
		distance := float64(lengthPool*countPool) / mInKm           // Дистанция при плавании
		speed := swimmingMeanSpeed(lengthPool, countPool, duration) // Скорость при плавании
...
func WalkingSpentCalories(action int, duration float64, weight, height float64) float64 {
	if duration <= 0 {
		return 0
	}
